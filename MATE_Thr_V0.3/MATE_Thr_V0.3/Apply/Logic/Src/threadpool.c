...
void HANDLE_MODE(void* parameter)
{
    ...
    while (1)
    {
        result = rt_mq_recv(HandleModemq, &HMInfo, sizeof(HandleModeInfo), timeout);
        if(result == RT_EOK)
        {
            // 收到数据，处理消息
            ...
            {
                // 清空 AutoModemq 队列
                while(rt_mq_recv(AutoModemq, &ClearBuf, sizeof(AutoModeInfo), RT_WAITING_NO) == RT_EOK);
...
void PlusControl(void* paramenter)
{
	...
	PWMInfo.PWMout[claw_catchSpeed] = 1250;
